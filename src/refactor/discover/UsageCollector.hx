...
class UsageCollector {
		...
		for (child in token.children) {
			switch (child.tok) {
				case Binop(OpArrow):
					...
					loopIdentifiers.push(ident);
				// case Kwd(KwdIn):
				// 	readExpression(context, ident, child.getFirstChild());
	...
	function readCaseConst(context:UsageContext, identifier:Identifier, token:TokenTree, scopeEnd:Int) {
		...
		for (child in pOpen) {
			switch (child.tok) {
				case BkOpen:
					readCaseArray(context, caseIdent, child, scopeEnd);
				case POpen:
	...
	function readAnonStructure(context:UsageContext, identifier:Identifier, token:TokenTree) {
		...
		for (child in token.children) {
			switch (child.tok) {
				...
				case Question:
					var ident:Identifier = makeIdentifier(context, child.getFirstChild(), TypedefField(fields), identifier);
					fields.push(Optional(ident));
				default:
			}
		}
